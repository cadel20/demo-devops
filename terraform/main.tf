...
resource "null_resource" "push_to_dockerhub" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      docker tag demo-devops:latest cadel2/demo-devops-app:latest
      
      echo ✅ Image taggée: cadel2/demo-devops-app:latest
      echo.
      echo 💡 Pour pousser sur Docker Hub:
      echo   1. docker login
      echo   2. docker push cadel2/demo-devops-app:latest
    EOT
  }
}
...
output "deployment_success" {
  ...
  docker tag demo-devops:latest cadel2/demo-devops-app:latest
  docker login
  docker push cadel2/demo-devops-app:latest
  EOT
}